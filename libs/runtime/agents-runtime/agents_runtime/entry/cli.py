"""CLI entry point: ``agents`` command.

Provides commands for agent execution and management:
    - ``agents run``         — Run an agent interactively
    - ``agents serve``       — Start data plane gRPC server
    - ``agents workspace``   — Manage workspace of agents
    - ``agents registry``    — CRUD for agent specs
"""
...
async def _cmd_run(args: argparse.Namespace) -> None:
    ...
    """
    from agents_runtime.manager.manager import AgentManager
    from agents_runtime.spec import AgentSpec, LaunchMode, RunConfig
...
async def _cmd_serve(args: argparse.Namespace) -> None:
    ...
    from agents_runtime.entry.server import serve
...
async def _cmd_workspace_up(args: argparse.Namespace) -> None:
    ...
    from agents_runtime.spec import WorkspaceConfig
...
async def _cmd_registry(args: argparse.Namespace) -> None:
    ...
    from agents_runtime.spec import AgentSpec
    ...
    if args.reg_command == "agent":
        ...
        print("Usage: agents registry agent {add|list|remove}")
...
async def _async_main(args: argparse.Namespace) -> None:
    ...
    if args.command == "run":
        ...
        if args.ws_command == "up":
            ...
            print("Usage: agents workspace {up|down|status}")
