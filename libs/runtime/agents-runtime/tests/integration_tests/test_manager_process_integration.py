...
from agents_runtime.generated import runtime_pb2_grpc
from agents_runtime.manager.manager import AgentManager
from agents_runtime.registry import Registry
from agents_runtime.spec import AgentSpec, AgentStatus, RunConfig, RuntimeEventType
...
def _patch_runtime_dependencies(
    *,
    ...
    with ExitStack() as stack:
        stack.enter_context(
            patch(
                "agents_runtime.manager.manager.get_checkpointer",
                checkpointer_factory,
            )
        )
        stack.enter_context(
            patch(
                "agents_runtime.agent.create_model",
        ...
        stack.enter_context(
            patch(
                "agents_runtime.agent.load_mcp_tools_from_configs",
                fake_load_mcp_tools,
            )
        )
        stack.enter_context(
            patch(
                "agents_runtime.agent.compute_summarization_defaults",
                return_value={
                    "trigger": ("messages", 10),
                    "keep": ("messages", 2),
        ...
        stack.enter_context(
            patch(
                "agents_runtime.agent.SummarizationMiddleware",
                return_value=object(),
            )
        )
        stack.enter_context(
            patch(
                "agents_runtime.agent.SummarizationToolMiddleware",
        ...
        if graph_factory is not None:
            stack.enter_context(
                patch(
                    "agents_runtime.agent.create_runtime_deep_agent",
