...
func (r *SQLiteRegistry) ListSkills(ctx context.Context) ([]domain.Skill, error) {
	page, err := r.ListSkillsPage(ctx, domain.PageQuery{})
	if err != nil {
		return nil, err
	}
...
// ListSkillsPage lists one page of skills ordered by name.
func (r *SQLiteRegistry) ListSkillsPage(
	ctx context.Context,
	query domain.PageQuery,
) (domain.ResourcePage[domain.Skill], error) {
	names, metadata, err := listNamesPage(ctx, r.db, "skills", query)
	if err != nil {
		return domain.ResourcePage[domain.Skill]{}, err
	}

	skills := make([]domain.Skill, 0, len(names))
	...
	return domain.ResourcePage[domain.Skill]{
		Items:        skills,
...
func (r *SQLiteRegistry) ListMCPConfigs(ctx context.Context) ([]domain.MCPConfig, error) {
	page, err := r.ListMCPConfigsPage(ctx, domain.PageQuery{})
	if err != nil {
		return nil, err
	}
	return page.Items, nil
...
// ListMCPConfigsPage lists one page of MCP configs ordered by name.
func (r *SQLiteRegistry) ListMCPConfigsPage(
	ctx context.Context,
	query domain.PageQuery,
) (domain.ResourcePage[domain.MCPConfig], error) {
	names, metadata, err := listNamesPage(ctx, r.db, "mcp_configs", query)
	if err != nil {
		return domain.ResourcePage[domain.MCPConfig]{}, err
	}

	configs := make([]domain.MCPConfig, 0, len(names))
	for _, name := range names {
	...
	return domain.ResourcePage[domain.MCPConfig]{
		Items:        configs,
		PageMetadata: metadata,
	}, nil
}
