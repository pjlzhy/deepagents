...
export type DebugDetailTab = 'payload' | 'metadata';
...
export function isTelemetryEvent(payload: unknown): payload is HTTPTelemetryEventDTO {
  return typeof payload === 'object' && payload !== null && ('event_type' in payload || 'stream_mode' in payload);
}
...
export function normalizeTelemetryEvent(
  ...
  return {
    ...
    streamMode: payload.stream_mode ?? 'unknown',
...
export function summarizeEvent(event: TelemetryEventVM): string {
  if (event.payload && typeof event.payload === 'object' && !Array.isArray(event.payload)) {
    ...
    if (typeof payload.text === 'string' && payload.text.trim()) return truncate(payload.text.trim());
